...
__all__ = ["HexBlockResult", "find_block", "block_candidates",
           "convex_hull", "min_area_quad"]
...
def find_block(scan: Path,
    ...
    """Where the chart is, judged by the ladder's own agreement measure.
    ...
    *current_corners* are passed only when somebody actually placed them. A
    ...
    for name, quad in cands:
        for k in range(4):
            ...
            if best is None or rho > best[0]:
                best = (rho, name, turned)
        if best is not None and best[1] == name:
            res.scores.append((name, round(best[0], 4)))
    if best is None:
    ...
    res.rho = rho
    res.route = name
    log.info("hex block search: %d quads, best %s at %.3f", len(cands), name, rho)
    return res
