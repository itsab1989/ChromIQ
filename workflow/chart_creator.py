...
class ChartCreator:
    ...
    def _engine_total_patches(self, params: "ChartParams") -> int | None:
        ...
        try:
            ...
            # For an ENGINE-ONLY instrument there is no printtarg to fall back
            # to, so swallowing this can only produce a wrong number or bury the
            # real reason under printtarg's refusal. Let it out: the Create
            # Chart tab already catches it and shows it to the user (#159).
            if params.instrument in ENGINE_ONLY_INSTRUMENTS:
                raise
            return None
    ...
    def _printtarg_done(
        ...
        tiffs = sorted({
            *stem_files(work_dir, stem, "*.tif", "*.TIF", "*.tiff"),
        })
    ...
    def _capture_printtarg_cht(self, work_dir: Path, stem: str,
        ...
        try:
            ...
            from core.file_manager import stem_files
            printed = len(stem_files(work_dir, stem, "_*.tif"))
    ...
    def _derive_geometry_from_render(self, work_dir: Path, stem: str) -> None:
        ...
        try:
            ti2 = work_dir / f"{stem}.ti2"
            from core.file_manager import stem_files
            tiffs = stem_files(work_dir, stem, "_*.tif")
