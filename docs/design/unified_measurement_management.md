...
### M-VERIFY-SOLIDS-REASON · PROPOSED · why a chart cannot answer the two solid colour metrics — the presets window, and the Measure tab pre-flight's metric list
...
headline is its name in this queue. It replaces "This chart carries no
...
yet how the sheet will be printed, so the line says both cases. Shown in the
windows while it waits, as M-VERIFY-UNCHECKED-METRICS is, because the line it
replaces was false.*

*It leaves M-VERIFY-PREFLIGHT's approved FROM PROFILE GAMUT paragraph as it
is. That paragraph still says those metrics "are judged against a colorimetric
reference", which is also no longer how they are worked out; a revision is
registered (B8-1374) and not made here.*
