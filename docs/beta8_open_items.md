...
### B8-179 · FROM PROFILE GAMUT freezes the window for seconds, with nothing on screen
- blocks release: no
- status: FIXED
...
- not fixed here: the work still runs ON the GUI thread and there is still no
  ...
  round was scoped out of. See B8-180.
...
### B8-180 · The reach query still runs on the GUI thread, with nothing on screen while it does
- blocks release: no
- status: OPEN
- found by: this round, measuring B8-179
- detail: B8-179 took the first click from 3.91 s to 1.66 s and a Margin change
...
### B8-181 · ChromIQ has two Install buttons and they disagreed about the name
- blocks release: no
- status: FIXED
...
### B8-182 · The installed name kept `CON`, `nul` and a 240-character file name
- blocks release: no
- status: FIXED
- found by: this round, while fixing B8-181; flagged in the beta-18 assessment
