...
### B8-78 · The patch-set editor is unusable for any CR30 chart, and the error box is taller than the screen
- blocks release: yes
- status: FIXED
...
- fix: three layers. (1) `ti2_relayout.PRINTTARG_INSTRUMENTS` /
  ...
  later, which for a CR30 is a pass that cannot succeed. (3) B8-79, which is
...
### B8-79 · "Save As" from the patch editor turned an engine chart into a printtarg chart
- blocks release: yes
- status: FIXED
...
- detail: `ui/warning_sign.py::warn` builds a `QMessageBox`, calls
  ...
  3055-character printtarg dump of B8-78: frame 420 x 1433 px on a 1079 px work
...
- detail: `paper_to_flag` falls through to an unbounded `f"{w:g}x{h:g}"` and
  ...
  through "Load patch set..." produced the same unbounded box as B8-78.
...
- fix: `check_printtarg_can_lay_out()` range-checks the custom `-p` at the same
  boundary as the instrument (B8-78). The four spin boxes take
...
- detail: `_pt_instr` offers only `i1` / `3p` / `CM`, so `findData("CR30")`
  ...
  caused is gone with B8-78 and B8-79: that chart no longer goes near printtarg,
