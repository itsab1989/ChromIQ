#!/usr/bin/env python3
"""D07: fresh-session Chart Layout default; clean auto-update order (build,
...
def save():
    L.save_json(R, L.LOGS / "d07_results.json")
...
def main() -> int:
    ...
    # ---------------- Chart Layout default in a fresh session (no project)
    ...
    L.set_combo_data(p.instr, "i1"); L.set_combo_data(p.paper, "A4"); L.set_combo_data(p.mode, "clip"); L.pump(600)
    R["chart-layout-default-fresh"] = {"align": p.patch_align.currentData(), "layout_mode": p.layout_mode.currentData(), "info": tab._manual_info_lbl.text()[-160:],
                                       "store": sorted(f.name for f in (Path(os.environ["CHROMIQ_PRESETS_DIR"]) / "Chart Layout").glob("*.json"))}
    L.log(f"fresh-session i1/A4/clip: {R['chart-layout-default-fresh']}")
    L.grab(win, OUT8 / "q32-fresh-session-i1-A4-clip.png"); save()
    # restore the sandbox store (remove the file D06 created)
    f = Path(os.environ["CHROMIQ_PRESETS_DIR"]) / "Chart Layout" / "i1_A4_clip.json"
    if f.exists():
        f.unlink(); L.log("  removed sandbox Chart Layout/i1_A4_clip.json (created by D06)")

    # ---------------- A7 clean order in B1-Guided
    L.open_project(win, settings, "B1-Guided"); L.click(tab._manual_btn); L.pump(500)
    if not tab._manual_engine_check.isChecked():
        L.set_check(tab._manual_engine_check, True)
    ...
    L.set_check(p.use_instr_margins, True); L.set_spin(p.area_min_patch, 0.0); L.set_spin(p.pages, 1)
    b0 = build(tab, watcher)
    builds = lambda: L.tab_log_text(tab).count("random start")  # noqa: E731
    if tab._auto_preview_check.isChecked():
        L.set_check(tab._auto_preview_check, False); L.pump(200)
    ...
    fired = L.wait_until(lambda: builds() > n0, 6000, "auto build after nudge (clean)"); t1 = round(time.time() - t0, 2)
    L.wait_build(tab, 120_000); L.pump(800)
    R["a7-clean"] = {"fired": fired, "after_s": t1, "flags": {"in_flight": tab._chart_build_in_flight(), "owned": getattr(tab, "_layout_owned_by_build", None), "last_sig_set": tab._last_auto_sig is not None}}
    L.log(f"A7 clean nudge: {R['a7-clean']}")
    # load preset B (generate immediately) then nudge
    c = tab._preset_combo
    ...
    n1 = builds(); t0 = time.time()
    L.set_spin(p.margins["t"], 20.0)      # preset B has own margins (editable)
    fired2 = L.wait_until(lambda: builds() > n1, 6000, "auto build after preset load"); t2 = round(time.time() - t0, 2)
    L.wait_build(tab, 120_000); L.pump(800)
    R["a7-after-preset"] = {"fired": fired2, "after_s": t2, "flags": {"in_flight": tab._chart_build_in_flight(), "owned": getattr(tab, "_layout_owned_by_build", None), "preset_ti1": str(getattr(tab, "_preset_ti1_path", None)), "pending_editor": str(getattr(tab, "_pending_editor_recipe", None))[:60]},
                           "info": tab._manual_info_lbl.text()[-200:]}
    L.log(f"A7 after preset: {R['a7-after-preset']}")
    c.setCurrentIndex(0); c.activated.emit(0); L.pump(600)
    n2 = builds(); L.set_spin(p.margins["t"], 22.0)
    fired3 = L.wait_until(lambda: builds() > n2, 6000, "auto build after none")
    L.wait_build(tab, 120_000); L.pump(500)
    R["a7-after-none"] = {"fired": fired3, "flags": {"in_flight": tab._chart_build_in_flight(), "preset_ti1": str(getattr(tab, "_preset_ti1_path", None))}}
    L.log(f"A7 after none: {R['a7-after-none']}")
    L.grab(win, OUT7 / "a30-clean-order.png")
    L.set_check(tab._auto_preview_check, False); L.pump(300); save()

    # ---------------- A10 extremes (Manual engine, i1 unless stated)
    tab._manual_target_name_edit.setText("A10-Extremes")
    # the name differs from the open project -> the TargetChangeDialog; choose create-and-keep
    L.set_combo_data(p.instr, "i1"); L.set_combo_data(p.paper, "A4"); L.set_combo_data(p.mode, "clip"); L.set_combo_data(p.layout_mode, "area_first")
    L.set_check(p.use_instr_margins, True); L.set_spin(p.area_min_patch, 0.0); L.set_spin(p.pages, 1)
    if auto.isChecked():
        auto.click(); L.pump(200)
    pw(tab, "targen", "-f").set_value(1); L.pump(400)
    est1 = L.panel_snapshot(tab)["layout_info_estimate"]
    R["x-1patch"] = {"estimate": est1, **build(tab, watcher, extra_expect=(("Rename Printer Profile", "Create \"A10-Extremes\" and keep"),))}
    L.log(f"1 patch: {R['x-1patch']}"); L.grab(win, OUT10 / "x01-1-patch.png"); save()
    pw(tab, "targen", "-f").set_value(10000); L.pump(400)
    est = L.panel_snapshot(tab)["layout_info_estimate"]
    R["x-10000"] = {"estimate": est, "pages_spin": p.pages.value(), **build(tab, watcher, timeout=300_000)}
    L.log(f"10000 patches: {R['x-10000']}"); L.grab(win, OUT10 / "x02-10000-patches.png"); save()
    if auto.isChecked() is False:
        auto.click(); L.pump(200)
