...
line and a sentence, and a graph with no limit says so. Driven ON SCREEN, on
any tree, so the same cases run before and after the change.

    CHROMIQ_DEMO_PACK=<pack> CHROMIQ_TREE=<tree> \\
        python drive_k47_graph_limits.py <out> <en|de> <case> [<case> ...]

A case is ``tag:project:run:runtype:type_id:set_id:ticks`` exactly as in
`drive_k45_pdf_layout.py`, whose steps this reuses: the report window from
the Tools menu entry, "New report…", the type, the set and the ticks, Generate
report. Then every trend tab that is shown is brought to the front and the
window photographed, with the key under the graph recorded, and the PDF is
saved through ChromIQ's own file dialog as ``<out>/<lang>-<tag>.pdf``.
    ...
    def s(d):
        ...
        for c in cases:
            ...
            for n, key in enumerate(TABS, 1):
                ...
                g["descriptions"] = [[k, t] for k, _c, t
                                     in chart.descriptions()]
                g["window_key"] = _key_text(dlg)
                d.shot(dlg, f"{tag}-{n:02d}-window-{key}")
            pdf = d.out / f"{tag}.pdf"
