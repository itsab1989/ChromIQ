        ...
        add_row(lgg, 0, tr("Create layout:"), self.layout_mode,
                tip=TooltipButton(
                    ...
                    tr("Two ways to decide patch size against how many fit. "
                       ...
                       "margins and the surrounding text possible at all. Two "
                       "things do need it, and both show up in the numbers:\n"
                       "   1. A clip border takes its own width on the side it "
                       "is printed on, so that margin is whichever is larger, "
                       "your number or the band. On the default sheet the band "
                       "is 26 mm wide and sits on the left, so a 5 mm left "
                       "margin comes out at 26; ask for 40 and you get 40. Put "
                       "the band on the right, under Clip-border content, and "
                       "the same happens to the right margin instead. Switch "
                       "it off and your number stands.\n"
                       "   2. Your instrument's own reserves. The strip "
                       "letters above the patches and the run-out below them "
                       "are not the same size as each other, so the block is "
                       "centred between them and the top and the bottom come "
                       "out a little larger than you asked, by different "
                       "amounts. The same is true at the far edge of the last "
                       "strip on a SpectroScan or a CR30.\n"
