        ...
        add_row(ogg, 2, "", self.export_pdf,
                tip=TooltipButton(
                    ...
                    tr("Saves a press-ready PDF of the chart next to the usual "
                       ...
                       "chart needs. The difference is what happens next: a TIFF is "
                       "a picture whose numbers get passed along, while PDF lets "
                       "whatever opens the file decide that DeviceRGB means some "
                       "particular colour space — and then convert it. Measured on "
                       "macOS, a pure red patch of 255,0,0 came back as 234,51,35; "
                       "on Linux the PDF interpreter behind most print queues "
                       "assigns a default colour profile to DeviceRGB unless told "
                       "not to; on Windows it depends entirely on the application "
                       "you print from. How far the colours move depends on the "
                       "machine and the program, so the same file can look fine on "
                       "one and be badly wrong on another.\n\n"
                       "Printing the PDF with colour management genuinely off is "
                       "perfectly possible — that is exactly what ChromIQ itself "
                       "does when it prints one. What you cannot do is assume it. "
                       "It works when the program you print from has an explicit "
                       "setting for it: a RIP with colour management switched off, "
                       "or a PDF application offering something like \"Same as "
                       "source (no colour management)\". It goes wrong in ordinary "
                       "viewers that simply draw the page and offer no such "
                       "setting.\n\n"
                       "So: if you have a RIP or an application with that setting, "
                       "the PDF is fine. If you are not certain, print the TIFF "
                       "sheets instead — they behave the same way everywhere. A "
                       "chart printed through a colour conversion measures the "
                       "conversion rather than your printer, and nothing afterwards "
