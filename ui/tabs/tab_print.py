...
_TT_BODY_PRINT_MACOS_NATIVE = (
    ...
    "ChromIQ locks it to ColorSync (application-managed colour), so it "
    "greys out and can't colour-manage the chart. You can ignore this "
    "pane.\n\n"
        ...
        if enabled:
            if _sys.platform == "win32":
                ...
                self._warn_lbl.setText(
                    ...
                    "dialog.\n\n"
                    "About the \"Color Matching\" pane (ColorSync vs the vendor's name): "
                    "ChromIQ locks it to ColorSync (application-managed colour), so it greys "
                    "out and — together with the driver-side colour controls (the ones that "
                    "actually shift pixels) — stays locked OFF. "
