...
def _project_with_conflict(tmp_path, name="Alpha", clash="Beta"):
    ...
    run.chart_ti2.write_text("the real chart", encoding="utf-8")
    (run.dir / f"{clash}.ti2").write_text("a stranger", encoding="utf-8")
    return proj, run
...
def test_the_real_file_keeps_the_projects_name(tmp_path):
    ...
    assert (d / "Beta.ti2").read_text(encoding="utf-8") == "the real chart"
...
def test_the_stranger_is_kept_under_a_name_that_explains_itself(tmp_path):
    ...
    assert aside.read_text(encoding="utf-8") == "a stranger", "nothing may be lost"
...
def test_renaming_twice_does_not_overwrite_the_first_file_moved_aside(tmp_path):
    ...
    d = tmp_path / "Beta" / "runs" / "run1"
    (d / "Gamma.ti2").write_text("a second stranger", encoding="utf-8")
    shutil.move(str(tmp_path / "Beta"), str(tmp_path / "Gamma"))
    Project.load(tmp_path / "Gamma").rename("Gamma")

    d = tmp_path / "Gamma" / "runs" / "run1"
    assert (d / "Gamma.ti2").read_text(encoding="utf-8") == "the real chart"
    kept = sorted(p.read_text(encoding="utf-8") for p in d.glob(f"*{CONFLICT_MARKER}*.ti2"))
