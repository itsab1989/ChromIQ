...
def test_percussive_completion_sounds_are_struck_not_hiss(stem):
    """#131 (Knut, then Basti): both were originally synthesised as flat white
    noise, so they sounded like static rather than drum strokes and clapping.
    The drumroll is now built from individual snare strokes; the applause is a
    CC0 recording of a real crowd (synthesis was tried twice and rejected by
    ear). Guard the properties that distinguish either from noise: a low
    zero-crossing rate (energy is not spread across the whole spectrum) and
...
def test_recorded_sound_is_documented_and_matches_the_pack_format():
    """#131: applause is the one file that is NOT generated by
    scripts/make_default_sounds.py. Its origin and CC0 dedication must stay
    documented, so the pack's licensing story remains auditable — and it must
    match the pack's audio format like every other file."""
    from core.resource_path import resource_path
    credits = resource_path("assets/sounds") / "CREDITS.md"
    assert credits.is_file(), "assets/sounds/CREDITS.md is missing"
    text = credits.read_text(encoding="utf-8")
    assert "applause.wav" in text and "CC0" in text, (
        "CREDITS.md must record the applause recording and its licence")
    gen = (resource_path("assets/sounds").parent.parent
           / "scripts" / "make_default_sounds.py")
    if gen.is_file():          # not shipped in the frozen bundle
        body = gen.read_text(encoding="utf-8").split('"""', 2)[-1]
        assert '"applause"' not in body, (
            "the generator must never overwrite the recorded applause.wav")
    _samples(resource_path("assets/sounds/task-complete") / "applause.wav")
