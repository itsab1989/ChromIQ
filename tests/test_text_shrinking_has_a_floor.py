...
def test_the_floor_is_eight_point_and_only_for_auto():
    assert tef.AUTO_SHRINK_FLOOR_PT == 8.0
    assert tef.text_floor_pt(0.0) == 8.0, "auto must stop at the 8 pt floor"
    assert tef.text_floor_pt(12.0) == 12.0, "a typed size is its own floor"
    assert tef.text_floor_pt(6.0) == 6.0, (
        "a typed size below 8 pt must be honoured, not raised to the floor")


def test_eight_point_is_the_same_paper_at_every_resolution():
    """A floor in POINTS is a floor on paper. The 9 px one was not."""
    for dpi in (150, 200, 300, 600, 1200):
        got = tef.px_to_pt(tef.pt_to_px(8.0, dpi), dpi)
        ...
        assert abs(got - 8.0) < 0.3, (dpi, got)
...
def test_knuts_own_numbers_now_earn_a_warning():
    ...
    line at the 8 pt floor plus its gap needs 3.076 mm. Under the 9 px floor
...
def test_auto_shrinking_stops_at_eight_point(monkeypatch):
...
def test_auto_still_shrinks_while_there_is_room(monkeypatch):
    ...
    # Both strips must be above the 8 pt floor (22 px at 200 dpi) and below
...
def test_a_typed_size_below_eight_point_works(monkeypatch):
...
def test_the_clip_border_text_stops_shrinking_at_eight_point(monkeypatch):
    ...
    assert min(sizes) >= floor, (
        ...
        f"the floor is 8 pt ({floor} px at 200 dpi)")
...
def test_the_clip_floor_is_the_same_paper_at_every_resolution(monkeypatch):
    ...
    for dpi in (200.0, 600.0):
        ...
        assert got >= 7.9, f"the clip text hit {got:.2f} pt at {dpi} dpi"


def test_a_typed_clip_size_is_printed_as_typed(monkeypatch):
    """Including below 8 pt, and without the fit loop stepping it down."""
    for pt in (6.0, 20.0):
...
def test_the_clip_inset_is_the_one_the_geometry_applies():
    ...
    assert tef.clip_text_squeeze(16.0, 4.0, 4).available_mm == pytest.approx(
        12.8), "the band's room is not what the renderer gives it"
...
def test_the_clip_squeeze_predicate_matches_the_renderer():
    """Four lines at 8 pt need 13.5 mm and a 16 mm band gives 12.8."""
    o = tef.clip_text_squeeze(16.0, 4.0, 4, 0.0, "right")
    assert o is not None, "a 16 mm band with four 8 pt lines does not warn"
    assert o.available_mm == pytest.approx(12.8)
    assert o.needed_mm > 12.8
