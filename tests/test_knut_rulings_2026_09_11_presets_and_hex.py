"""Knut's #182 posts of 2026-09-11: the preset seed tag and the hexagon help.

**Items A and B, the custom paper name and the Square word, are NOT here.** Two
rounds answered them on two bases and both landed; the one that kept its answer
is `test_knut_rulings_2026_09_11.py`, which factors the orientation word through
`data.patch_db` so the patch-set editor's own name generator cannot answer
differently. What is left below is what only this round did.
...
# ---------------------------------------------------------------------------
# THE SEED TAG IS NOT STAMPED INTO THE PRESETS, AND THAT IS DELIBERATE.
#
# Knut asked for the tag on every built-in with the box OFF, and said *"We
# would like NOT to do this manually for all presets."* This round stamped
# `"seed_fixed": False` into all 141 bundled recipes with a script. The branch
# it merged into already answered him from one constant applied in code, so
# every built-in already loaded with the box unticked and NOT ONE PRESET FILE
# WAS EDITED — which serves his other sentence, *"All seed numbers stored in
# the presets should be as they are today"*, better than any edit can: nothing
# was touched, so nothing could be disturbed.
#
# The tests that asserted the stamping are therefore gone rather than adapted.
# What he asked for is checked in `test_knut_rulings_2026_09_11.py`:
# `test_no_builtin_preset_recipe_hand_carries_the_tag` holds the mechanism, and
# `test_selecting_a_prebuilt_file_preset_unticks_the_box` holds the outcome.
