...
least 75 % of it (Knut, 5789263863, approved in 5789539407; B8-828).
...
def test_knuts_formula_on_the_margins():
    ...
    (210-26-6) x (297-38-19) / (210 x 297) = 178 x 240 / 62370.

    MUTATION: leave the right and bottom margins out of `coverage_of` and it
    reads 184 x 259 / 62370 = 76.4 %, a page that passes."""
...
def test_the_floor_is_75_percent_and_inclusive(tmp_path):
    """Exactly 75 %: counted. A hair under: left out, N-A with the reason.

    MUTATION: `>` for `>=` against `EVENNESS_MIN_PAGE_COVERAGE` in
    `evenness_from_residuals` and the page at exactly 75 % is refused."""
    import numpy as np
    assert MR.EVENNESS_MIN_PAGE_COVERAGE == 0.75
    grid = MR.evenness_grid_from_layout([12], 12, 144, coverage=[0.75])
    ev = MR.evenness_from_residuals(grid, np.zeros((144, 3)), shuffles=20)
    assert ev["eligible"], ev
    grid = MR.evenness_grid_from_layout([12], 12, 144, coverage=[0.7499])
    ev = MR.evenness_from_residuals(grid, np.zeros((144, 3)), shuffles=20)
    assert ev["reason"] == MR.REASON_EVENNESS_PAGE_COVERAGE
    # …and through a real report, from the geometry beside the chart
    ti2, where = _write_chart(tmp_path / "a", coverage=0.752)
    ev = _ev(ti2, where)
    assert ev["eligible"], ev
    assert ev["coverage"][0] == pytest.approx(0.752, abs=2e-3)
    ti2, where = _write_chart(tmp_path / "b", coverage=0.745)
    ev = _ev(ti2, where)
...
def test_the_floor_is_one_constant(tmp_path, monkeypatch):
    """Lower the constant and a 68 % page is counted, and every sentence
    quoting the floor says the new figure.

    MUTATION: write 0.75 into `evenness_from_residuals` or 75 into the note
    ...
    ti2, where = _write_chart(tmp_path / "a", coverage=0.68)
    rep = MR.build_report(_write_sheet(ti2, where, _even(0.2)))
    assert "at least 75 % is needed" in _evenness_coverage_sentence(rep)
    monkeypatch.setattr(MR, "EVENNESS_MIN_PAGE_COVERAGE", 0.6)
    ev = _ev(ti2, where)
    assert ev["eligible"], ev
    ti2, where = _write_chart(tmp_path / "b", coverage=0.55)
    rep = MR.build_report(_write_sheet(ti2, where, _even(0.2)))
...
def test_an_uncovered_page_is_left_out_and_the_others_judged(tmp_path):
    ...
    assert "The patches on page 2 cover 60.0 % of the page, less than 75 %, " \
...
def test_the_note_names_the_page_and_its_share_rounded_down():
    ...
    X % of the page; at least 75 % is needed". A page at 74.96 % must never
    read "75.0 %" beside a refusal.

    MUTATION: round the share instead of flooring it in `_coverage_pct`."""
    from ui.dialogs.measurement_report_dialog import _evenness_coverage_sentence
    one = {"evenness": {"pages_uncovered": [1], "coverage": [0.74962]}}
    assert _evenness_coverage_sentence(one) == (
        "the patches on page 1 of the measured chart cover 74.9 % of the "
        "page; at least 75 % is needed")
    two = {"evenness": {"pages_uncovered": [1, 2], "coverage": [0.653, 0.66]}}
    assert _evenness_coverage_sentence(two) == (
        "the patches on pages 1, 2 of the measured chart cover at most 66.0 % "
        "of their page; at least 75 % is needed")
...
def test_the_report_window_shows_the_note_on_both_rows(qapp, tmp_path):
    ...
    ti2, where = _write_chart(tmp_path / "c", coverage=0.683)
    ...
    try:
        ...
        text = _visible(dlg)
        want = ("the patches on page 1 of the measured chart cover 68.3 % of "
                "the page; at least 75 % is needed")
...
def test_a_built_chart_reads_what_measured_from_preview_shows(tmp_path):
    ...
    the chart existed. 68.4 %: the i1Pro's 26 mm clip border and 38 mm top
    leave an A4 page no room for 75 %.
...
def test_the_presets_window_and_the_preflight_ask_the_same_floor():
    """An engine preset that has not been built: the prediction says 68.4 %,
    so the presets window and the pre-flight (`assess_any`) both file the
    two rows under the new reason, with a line of their own, and the star is
    not touched (a layout shortfall).
    ...
    p = next(x for x in KNUT_PRESETS
             if x.slug == "i1_w8_a4_572p_1page_portrait_w8_0mm")
    chart = Path(resource_path(p.ti1_asset))
    a = dict(PE.assess(chart, MR.REPORT_TYPE_FULL, "chromiq_default",
    ...
    assert "75 %" in reason_line(MR.REASON_EVENNESS_PAGE_COVERAGE)
...
def test_the_help_text_quotes_the_floor():
    """The metric help icon says 75 %, and the constant must agree.
