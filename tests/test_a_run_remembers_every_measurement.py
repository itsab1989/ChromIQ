...
def test_every_measurement_of_a_run_is_its_own_row(tmp_path, qapp, monkeypatch):
    ...
    try:
        # **EVERY MEASUREMENT TICKED, EXPLICITLY (B8-490, B8-590).** The
        # window opens on a saved report, and since Knut's beta-25 ruling a
        # saved report's own settings and its own MEASUREMENT ticks come back
        # with it — a per-measurement record is about one date, so the window
        # comes up covering that one. This check is about how many rows the run
        # HAS, so it asks for all of them.
        #
        # It used to be `dlg._all_runs_check.setChecked(True)`. That box, and
        # the feature behind it, were removed on Knut's 2026-09-20 ruling
        # (B8-590): *"only the selected/ticked measurements shall be part of
        # the report when created/updated (always)."* "Select all" is the
        # button he asked for in its place.
