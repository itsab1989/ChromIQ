...
def test_reset_chart_artefacts_wipes_exports_and_cache_keeps_reports(fm) -> None:
    ...
    run.ensure_exports_dir().joinpath("x-colours.txt").write_text("x", encoding="utf-8")
    run.ensure_cache_dir().joinpath("x-sample.cht").write_text("x", encoding="utf-8")
    run.ensure_reports_dir().joinpath("Quality_Check_1_x.txt").write_text("x", encoding="utf-8")
    run.preconditioning_ti3.write_text("keep", encoding="utf-8")
