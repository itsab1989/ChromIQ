...
def test_the_busy_bar_is_five_action_cells_in_neutral(wearing, qapp):
    """Asserted on the PAINTED bar, not on a palette helper.

    Two branches gave this bar its Neutral answer independently and both were
    right; only one helper survived the merge, and a test written against the
    other one broke while the pixels never moved. So this asks the picture.
    """
    from ui.spectrum_progress import SpectrumSegmentsBar, _palette
    wearing(APPEARANCE_NEUTRAL)
    bar = SpectrumSegmentsBar()
    bar.resize(240, 40)
    bar.set_label("BUILDING", "colprof")
    bar.set_value(0.6)
    assert not _grab_has_hue(bar), "the busy bar still paints hues in Neutral"
    label, sub, filled, empty = _palette()
    assert (label, sub) == (nm.NM_TEXT_DIM, nm.NM_TEXT_FAINT)
    assert filled.name() == nm.NM_ACTION
...
def test_the_busy_bar_keeps_the_spectrum_in_light_and_dark(wearing, qapp):
    from ui.spectrum_progress import (LABEL_COLOR, SPECTRUM, SUBLABEL_COLOR,
                                      SpectrumSegmentsBar, _palette)
    for mode in (APPEARANCE_LIGHT, APPEARANCE_DARK):
        wearing(mode)
        bar = SpectrumSegmentsBar()
        bar.resize(240, 40)
        bar.set_value(1.0)
        assert _grab_has_hue(bar), f"{mode} lost the spectrum"
        assert _palette()[:2] == (LABEL_COLOR, SUBLABEL_COLOR)
