...
def test_the_chart_builder_asks_for_the_numeric_inverse(fn):
    ...
    src = inspect.getsource(getattr(gamut_target, fn, None) or gamut_target)
    assert "numeric_inverse=True" in src, (
        f"{fn} inverts through the baked B2A table again; a chart built that "
        "way lands 0.366 dE00 from its own aims instead of 0.052")
